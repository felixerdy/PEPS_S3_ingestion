...
import prepareToBeam as beamer
...
#====================
email = auth.peps_mail
passwd = auth.peps_pass
    ...
    for prod in download_dict.keys():
        ...
        if (not(options.no_download) and not(file_exists)):
            if storage_dict[prod] == "tape":
                # downloading product from tape requires several attemps,
                # waiting for the tape to be read
            ...
            beamer.beamMeUpScotty("%s/%s.zip" % (options.write_dir, prod))
